...
export function Reports({catalog,api,status}:{catalog:Catalog;api:Api;status:boolean}) {
 ...
 const ppView=status&&entity==='__pp__'
 const ppIds=new Set(catalog.parties.filter(p=>p.kind==='pp').map(p=>p.id))
 const sourceRows=ppView?data?.accounts.filter(a=>ppIds.has(catalog.accounts.find(x=>x.id===a.account_id)?.party_id||'')).map(a=>({...a,opening:decimal(-cents(a.opening)),debit:a.credit,credit:a.debit,closing:decimal(-cents(a.closing))})):data?.accounts
 const selected=data?.pnl.find(p=>p.entity_id===entity)||data?.consolidated
 const rows=sourceRows?.filter(a=>(catalog.accounts.find(x=>x.id===a.account_id)?.active!==false||(status?cents(a.closing)!==0n:(cents(a.debit)!==0n||cents(a.credit)!==0n)))&&(!entity||ppView||a.entity_id===entity)&&(status?['bank','cash','party']:['income','expense']).includes(a.kind)&&(zero||(status?cents(a.closing)!==0n:(cents(a.debit)!==0n||cents(a.credit)!==0n))))||[]
 const receivable=rows.reduce((sum,a)=>sum+(cents(a.closing)>0n?cents(a.closing):0n),0n),payable=rows.reduce((sum,a)=>sum+(cents(a.closing)<0n?-cents(a.closing):0n),0n)
 const statusBalances=status&&!ppView?catalog.entities.filter(e=>entity?e.id===entity:e.active).map(e=>({entity:e,total:(sourceRows||[]).filter(a=>a.entity_id===e.id&&['bank','cash','party'].includes(a.kind)).reduce((sum,a)=>sum+cents(a.closing),0n)})):[]
 ...
 const tableRows=rows.map(a=>[catalog.entities.find(e=>e.id===a.entity_id)?.name||'',ppView?'ลูกหนี้ / เจ้าหนี้ของ PP':catalog.accounts.find(x=>x.id===a.account_id)?.name||'',a.opening,a.debit,a.credit,a.closing])
 return <section className="card"><h2>{status?'สถานะเงิน / ลูกหนี้–เจ้าหนี้':'รายงานรายรับ–รายจ่าย'}</h2><Filters includePP={status} catalog={catalog} from={from} to={to} entity={entity} change={(f,t,e)=>{setFrom(f);setTo(t);setEntity(e);setAccount('')}}/>
  {status&&<p>{ppView?'มุมมอง PP · บวก = กิจการค้าง PP · ลบ = PP ค้างกิจการ · เฉพาะยอดที่บันทึกในแอปนี้':'ยอดคงเหลือถึงวันที่เลือก · ลูกหนี้เป็นบวก เจ้าหนี้เป็นลบ · คู่ระหว่างกิจการเป็นยอดตรงข้ามกัน ไม่ใช่รายได้'}</p>}
  {!data?<p role="status">{error||'กำลังโหลด…'}</p>:<>
   {ppView&&<div className="summary-grid"><div className="review"><small>ลูกหนี้ของ PP รวม</small><strong>{money(decimal(receivable))}</strong></div><div className="review"><small>เจ้าหนี้ของ PP รวม</small><strong>{money(decimal(payable))}</strong></div></div>}
   {statusBalances.length>0&&<><div className="summary-grid">{statusBalances.map(({entity:statusEntity,total})=><div className="review" key={statusEntity.id}><small>{statusEntity.name} · ยอดสุทธิ</small><strong className={total<0n?'negative':''}>{money(decimal(total))}</strong><small>{total>0n?'บวก':total<0n?'ลบ':'ศูนย์'} · หลังหักเงินและยอดค้างทุกบัญชี</small></div>)}</div><p>ยอดสุทธิ = เงินธนาคาร + เงินสด + ลูกหนี้–เจ้าหนี้ทุกคู่ ถึงวันที่เลือก</p></>}
   ...
   <div className="toolbar"><label className="check"><input type="checkbox" checked={zero} onChange={e=>setZero(e.target.checked)}/>แสดงบัญชียอดศูนย์</label><CsvExport name={`moana-${from}-${to}.csv`} label="ดาวน์โหลด CSV" rows={status?[['ช่วงวันที่',from,to],['มุมมอง',ppView?'PP':'กิจการ'],['กิจการ','บัญชี','ยกมา','Dr','Cr','คงเหลือ'],...tableRows]:[['ช่วงวันที่',from,to],['กิจการ','บัญชี','ยอดในช่วงวันที่'],...rows.map(a=>[catalog.entities.find(e=>e.id===a.entity_id)?.name||'',catalog.accounts.find(x=>x.id===a.account_id)?.name||'',periodValue(a)])]}/></div>
   <div className="table-scroll"><table><thead><tr><th>กิจการ / บัญชี</th><th>{status?'คงเหลือ (บาท)':'ยอดในช่วงวันที่ (บาท)'}</th><th>รายละเอียด</th></tr></thead><tbody>{rows.map(a=><tr key={a.account_id}><td>{catalog.entities.find(e=>e.id===a.entity_id)?.name}<small>{ppView?(cents(a.closing)>0n?'ค้าง PP':cents(a.closing)<0n?'PP ค้างกิจการนี้':'ไม่มียอดค้าง'):catalog.accounts.find(x=>x.id===a.account_id)?.name}</small></td><td className={cents(status?a.closing:periodValue(a))<0n?'negative':''}>{money(status?a.closing:periodValue(a))}</td><td><button className="secondary" onClick={()=>setAccount(a.account_id)}>ดูบัญชี</button></td></tr>)}</tbody></table>{!rows.length&&<p>ไม่มีความเคลื่อนไหวหรือยอดคงเหลือในตัวกรองนี้</p>}</div>
   {account&&<GeneralLedger key={`${account}-${from}-${to}`} ppView={ppView} account={account} from={from} to={to} api={api} catalog={catalog}/>}
  </>}
 </section>
}
...
function GeneralLedger({account,from,to,api,catalog,ppView=false}:{account:string;from:string;to:string;api:Api;catalog:Catalog;ppView?:boolean}) {
 const [rawData,setData]=useState<GL|null>(null),[error,setError]=useState(''),[detail,setDetail]=useState('')
 ...
 const data=rawData&&ppView?{...rawData,opening:decimal(-cents(rawData.opening)),closing:decimal(-cents(rawData.closing)),lines:rawData.lines.map(l=>({...l,debit:l.credit,credit:l.debit,balance:decimal(-cents(l.balance))}))}:rawData
 const title=ppView?`PP ↔ ${catalog.entities.find(e=>e.id===catalog.accounts.find(a=>a.id===account)?.entity_id)?.name}`:catalog.accounts.find(a=>a.id===account)?.name
 return <div className="review"><h3>{title} · บัญชีแยกประเภท</h3>{!data?<p>{error||'กำลังโหลด…'}</p>:<><p>ยกมา {money(data.opening)} · คงเหลือ {money(data.closing)}</p><CsvExport name="moana-ledger.csv" label="ดาวน์โหลดบัญชี CSV" rows={[[title||'',from,to],['ยอดยกมา',data.opening],['วันที่','เลขที่','ฉบับ','ประเภท','Dr','Cr','คงเหลือ'],...data.lines.map(l=>[l.date,l.transaction_id,String(l.revision),l.role,l.debit,l.credit,l.balance])]}/><div className="table-scroll"><table><thead><tr><th>วันที่ / รายการ</th><th>Dr</th><th>Cr</th><th>คงเหลือ</th></tr></thead><tbody>{data.lines.map(l=><tr key={l.id}><td><button className="secondary" onClick={()=>setDetail(l.transaction_id)}>{l.date} · {l.transaction_id.slice(0,8)}</button><small>{l.role} · ฉบับ {l.revision}</small></td><td>{money(l.debit)}</td><td>{money(l.credit)}</td><td>{money(l.balance)}</td></tr>)}</tbody></table></div></>}{detail&&<>{ppView&&<p>รายการต้นทางด้านล่างแสดงบัญชีฝั่งกิจการ</p>}<Detail id={detail} catalog={catalog} api={api}/></>}</div>
}
